...
public class Normalize {
    ...
    public static class NormalizeMapper extends Mapper<LongWritable, Text, Text, Text> {
        ...
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            //EXAMPLE:
            //input value:  movieA:movieB \t relation
            //collect the relationship list for movieA
            String[] inputs = value.toString().trim().split("\t");
            String movieA = inputs[0].toString().trim().split(":")[0];
            String movieB = inputs[0].toString().trim().split(":")[1];

            context.write(new Text(movieA), new Text(movieB + ":" + inputs[1]));
        }
    }
    ...
    public static class NormalizeReducer extends Reducer<Text, Text, Text, Text> {
        ...
        public void reduce(Text key, Iterable<Text> values, Context context)
            ...
            //input key = movieA
            //input value = <mov ieB1:relation1, movieB2:relation2...>
            //output key: movieB1, ..
            //output value: movieA=relation1/denominator, ...
            int denominator = 0;
